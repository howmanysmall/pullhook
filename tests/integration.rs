...
#[test]
fn categories_descriptions_only_prints_clean_descriptions() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["categories", "--descriptions-only"]);

	assert!(output.status.success(), "categories --descriptions-only should succeed");
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
		vec![
	...
	assert!(
		stderr.trim().is_empty(),
		"categories --descriptions-only should not write stderr"
	);
}

#[test]
fn categories_search_filter_composes_with_descriptions_only() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(
		temp.path(),
		&["categories", "--search", "repository", "--descriptions-only"],
	);

	assert!(
		output.status.success(),
		"categories --search repository --descriptions-only should succeed"
	);
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
		vec!["Commands that inspect repository or config state."]
	);
	let stderr = stderr_text(&output);
	assert!(
		stderr.trim().is_empty(),
		"categories --search repository --descriptions-only should not write stderr"
	);
}
...
fn categories_line_outputs_conflict_with_json() {
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"category line-output mode should conflict with --json for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
		assert!(stderr.contains("--json"));
...
fn examples_text_lists_common_workflows() {
	...
	let output = run_pullhook(temp.path(), &["examples"]);
...
#[test]
fn examples_json_lists_common_workflows() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["examples", "--json"]);

	assert!(
		output.status.success(),
		"examples --json should succeed outside a git repo"
	);
...
#[test]
fn examples_command_names_only_prints_clean_command_names() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["examples", "--command-names-only"]);

	assert!(output.status.success(), "examples --command-names-only should succeed");
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
		vec![
	...
	assert!(
		stderr.trim().is_empty(),
		"examples --command-names-only should not write stderr"
	);
}

#[test]
fn examples_search_filter_composes_with_command_names_only() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(
		temp.path(),
		&["examples", "--search", "install", "--command-names-only"],
	);
	...
	assert!(
		stderr.trim().is_empty(),
		"filtered examples --command-names-only should not write stderr"
	);
}

#[test]
fn examples_titles_only_prints_clean_titles() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["examples", "--category", "reference", "--titles-only"]);

	assert!(
		output.status.success(),
		"examples --category reference --titles-only should succeed"
	);
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
	...
	assert!(
		stderr.trim().is_empty(),
		"examples --category reference --titles-only should not write stderr"
	);
}
...
#[test]
fn examples_categories_only_prints_clean_example_categories() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["examples", "--categories-only"]);

	assert!(output.status.success(), "examples --categories-only should succeed");
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
		vec!["workflow", "diagnostic", "generator", "reference"]
	...
	assert!(
		stderr.trim().is_empty(),
		"examples --categories-only should not write stderr"
	);
}

#[test]
fn examples_search_filter_composes_with_categories_only() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["examples", "--search", "install", "--categories-only"]);

	assert!(
		output.status.success(),
		"examples --search install --categories-only should succeed"
	);
	let stdout = stdout_text(&output);
	assert_eq!(stdout.lines().collect::<Vec<_>>(), vec!["workflow", "reference"]);
	let stderr = stderr_text(&output);
	assert!(
		stderr.trim().is_empty(),
		"filtered examples --categories-only should not write stderr"
	);
}
...
#[test]
fn examples_command_names_only_conflicts_with_other_output_modes() {
	let temp = tempfile::tempdir().expect("create temp dir");
	let conflicting_modes: &[&[&str]] = &[
		...
		&["examples", "--command-names-only", "--categories-only"],
	];
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"examples --command-names-only should conflict for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
		assert!(stderr.contains("--command-names-only"));
	}
}

#[test]
fn examples_titles_only_conflicts_with_other_output_modes() {
	let temp = tempfile::tempdir().expect("create temp dir");
	let conflicting_modes: &[&[&str]] = &[
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"examples --titles-only should conflict for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
...
fn examples_summaries_only_conflicts_with_other_output_modes() {
	...
	let conflicting_modes: &[&[&str]] = &[
		...
		&["examples", "--summaries-only", "--categories-only"],
	];
...
#[test]
fn examples_categories_only_conflicts_with_other_output_modes() {
	let temp = tempfile::tempdir().expect("create temp dir");
	let conflicting_modes: &[&[&str]] = &[
		...
		&["examples", "--categories-only", "--summaries-only"],
	];
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"examples --categories-only should conflict for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
...
#[test]
fn commands_categories_only_prints_clean_command_categories() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["commands", "--categories-only"]);

	assert!(output.status.success(), "commands --categories-only should succeed");
	let stdout = stdout_text(&output);
	assert_eq!(
		stdout.lines().collect::<Vec<_>>(),
		vec!["workflow", "diagnostic", "generator", "reference"]
	...
	assert!(
		stderr.trim().is_empty(),
		"commands --categories-only should not write stderr"
	);
}

#[test]
fn commands_search_filter_composes_with_categories_only() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["commands", "--search", "doctor", "--categories-only"]);

	assert!(
		output.status.success(),
		"commands --search doctor --categories-only should succeed"
	);
	let stdout = stdout_text(&output);
	assert_eq!(stdout.lines().collect::<Vec<_>>(), vec!["diagnostic"]);
	let stderr = stderr_text(&output);
	assert!(
		stderr.trim().is_empty(),
		"filtered commands --categories-only should not write stderr"
	);
}
...
fn commands_summaries_only_conflicts_with_other_output_modes() {
	...
	let conflicting_modes: &[&[&str]] = &[
		...
		&["commands", "--summaries-only", "--categories-only"],
	];
...
#[test]
fn commands_categories_only_conflicts_with_other_output_modes() {
	let temp = tempfile::tempdir().expect("create temp dir");
	let conflicting_modes: &[&[&str]] = &[
		&["commands", "--categories-only", "--json"],
		&["commands", "--categories-only", "--names-only"],
		&["commands", "--categories-only", "--summaries-only"],
	];
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"commands --categories-only should conflict for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
		assert!(stderr.contains("--categories-only"));
...
fn commands_example_commands_only_conflicts_with_other_output_modes() {
	...
	let conflicting_modes: &[&[&str]] = &[
		...
		&["commands", "--example-commands-only", "--markdown"],
...
#[test]
fn commands_markdown_prints_filtered_command_table() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["commands", "--category", "diagnostic", "--markdown"]);

	assert!(
		output.status.success(),
		"commands --category diagnostic --markdown should succeed"
	);
	let stdout = stdout_text(&output);
	let lines = stdout.lines().collect::<Vec<_>>();
	assert_eq!(
		lines.first().copied(),
		Some("| Command | Category | Repo required | Summary | Examples |")
	);
	assert!(
		lines.contains(
			&"| `rules` | `diagnostic` | yes | List configured rule and group names. | `pullhook rules --commands-only`<br>`pullhook rules --count-only`<br>`pullhook rules --search lint --names-only` |"
		),
		"diagnostic markdown table should include rules examples"
	);
	assert!(
		!stdout.contains("| `run` | `workflow` |"),
		"category filter should exclude workflow commands"
	);
	let stderr = stderr_text(&output);
	assert!(stderr.trim().is_empty(), "commands --markdown should not write stderr");
}

#[test]
fn commands_markdown_includes_matching_top_level_examples() {
	let temp = tempfile::tempdir().expect("create temp dir");

	let output = run_pullhook(temp.path(), &["commands", "--search", "install", "--markdown"]);

	assert!(
		output.status.success(),
		"commands --search install --markdown should succeed"
	);
	let stdout = stdout_text(&output);
	assert!(stdout.contains("| `managers` | `reference` | no |"));
	assert!(stdout.contains("Top-level examples:"));
	assert!(
		stdout
			.contains("- `pullhook --install`: Use legacy one-off mode to detect the package manager and run install.")
	);
	let stderr = stderr_text(&output);
	assert!(
		stderr.trim().is_empty(),
		"commands --search install --markdown should not write stderr"
	);
}

#[test]
fn commands_markdown_conflicts_with_other_output_modes() {
	let temp = tempfile::tempdir().expect("create temp dir");
	let conflicting_modes: &[&[&str]] = &[
		&["commands", "--markdown", "--json"],
		&["commands", "--markdown", "--names-only"],
		&["commands", "--markdown", "--summaries-only"],
		&["commands", "--markdown", "--categories-only"],
		&["commands", "--markdown", "--example-commands-only"],
	];
	...
	for args in conflicting_modes {
		...
		assert!(
			!output.status.success(),
			"commands --markdown should conflict for args {args:?}"
		);
		let stderr = stderr_text(&output);
		assert!(stderr.contains("cannot be used with"));
		assert!(stderr.contains("--markdown"));
...
fn commands_help_describes_search_and_output_modes() {
	...
	assert!(commands_stdout.contains("Print a Markdown command reference table"));
	...
	assert!(commands_stdout.contains("pullhook commands --markdown"));
}
