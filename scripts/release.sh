...
current_ver=$(cargo metadata --format-version 1 --no-deps 2> /dev/null \
    | grep -m1 '"name":"pullhook"' -A1 | tail -1 | sed -n 's/.*"version":"\([^"]*\)".*/\1/p')
...
green "  Monitor progress at: https://github.com/howmanysmall/pullhook/actions"
